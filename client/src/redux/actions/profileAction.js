...
export const updateProfileUser = ({userData,avatar, auth}) => async (dispatch) => {
    ...
    if(userData.fullname.length > 25)
        return dispatch({type: GLOBALTYPES.ALERT,payload: {error: "Your full name must contain at max 25 characters."}})

    if(userData.story.length > 200)
        return dispatch({type: GLOBALTYPES.ALERT,payload: {error: "Your story must be of at max 200 characters."}})
